<<<<<<< HEAD
-- Full gridding of poblacion, zoom 0

=======
/*

    Grids the zoom 0 of population
...
create table poblacion_processing.grid_ieca_agg_zoom0 as
select
...
create index grid_ieca_agg_zoom0_geom_gist
on poblacion_processing.grid_ieca_agg_zoom0
using gist(geom);
...
create table poblacion_processing.grid_ieca_agg_zoom0_json as
select
...
from
    poblacion_processing.grid_ieca_agg_zoom0 a inner join
    context_grid.zoom0 b
    on a.cell_gid = b.gid;

create index grid_ieca_agg_zoom0_json_cell_gid
on poblacion_processing.grid_ieca_agg_zoom0_json
...
drop table if exists temp.pob_zoom0_gridding;


-- Creates the temp data

create table temp.pob_zoom0_gridding as
...
from
    ...
    context_grid.zoom0 a inner join
...
from poblacion_processing.grid_ieca_agg_zoom0_json;
=======
        dat0
    )::cell__cell
))
