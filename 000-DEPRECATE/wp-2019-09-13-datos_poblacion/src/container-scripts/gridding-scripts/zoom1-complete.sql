<<<<<<< HEAD
-- Full gridding of poblacion, zoom 1

=======
/*

    Grids the zoom 1 of population
...
create table poblacion_processing.grid_ieca_agg_zoom1 as
select
...
create index grid_ieca_agg_zoom1_geom_gist
on poblacion_processing.grid_ieca_agg_zoom1
using gist(geom);
...
create table poblacion_processing.grid_ieca_agg_zoom1_json as
select
...
from
    poblacion_processing.grid_ieca_agg_zoom1 a inner join
    context_grid.zoom1 b
    on a.cell_gid = b.gid;

create index grid_ieca_agg_zoom1_json_cell_gid
on poblacion_processing.grid_ieca_agg_zoom1_json
...
drop table if exists temp.pob_zoom1_gridding;


-- Creates the temp data

create table temp.pob_zoom1_gridding as
...
from
    ...
    context_grid.zoom1 a inner join
...
from poblacion_processing.grid_ieca_agg_zoom1_json;
=======
        dat0
    )::cell__cell
))
